...
public class CombinedStateCalculation extends AbstractPatientCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		EhrReportConstants.OccurenceStates state = (EhrReportConstants.OccurenceStates) parameterValues.get("state");
		...
		for (Integer pId : cohort) {
			...
			if ((state.equals(EhrReportConstants.OccurenceStates.REVISIT)) && revisits.contains(pId)) {
				isCandidate = true;
			} else if ((state.equals(EhrReportConstants.OccurenceStates.NEW)) && newPatients.contains(pId)) {
				isCandidate = true;
			}
