...
public class CombinedStateCalculation extends AbstractPatientCalculation {
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		Moh717Constants.OccurenceStates state = (Moh717Constants.OccurenceStates) parameterValues.get("state");
		...
		NewPatientOccurenceCalculation newPatientsCalculation = Context.getRegisteredComponents(
		    NewPatientOccurenceCalculation.class).get(0);
		...
		for (Integer pId : cohort) {
			...
			if ((state.equals(Moh717Constants.OccurenceStates.REVISIT)) && revisits.contains(pId)) {
				isCandidate = true;
			} else if ((state.equals(Moh717Constants.OccurenceStates.NEW)) && newPatients.contains(pId)) {
				isCandidate = true;
			}
