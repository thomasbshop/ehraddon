...
import org.openmrs.module.ehraddons.reporting.calculation.BooleanResult;
import org.openmrs.module.ehraddons.reporting.calculation.CalculationWithResultFinder;
...
public class EhrCalculationUtils {
	...
	public static CalculationResultMap evaluateWithReporting(DataDefinition dataDefinition, Collection<Integer> cohort,
		...
		try {
			...
			if (dataDefinition instanceof PersonDataDefinition) {
				...
				throw new IllegalArgumentException("Unknown DataDefinition type: " + dataDefinition.getClass());
			}
	 ...
	 */
	public static Set<Integer> patientsThatPass(CalculationResultMap results, Object requiredResult,
	        CalculationWithResultFinder resultFinder, EvaluationContext context) {
		...
		for (Map.Entry<Integer, CalculationResult> e : results.entrySet()) {
			...
			boolean nullResultFinder = CalculationWithResultFinder.NULL.equals(resultFinder) && result.getValue() == null
			        && requiredResult == null;
			boolean defaultResultFinder = CalculationWithResultFinder.DEFAULT.equals(resultFinder)
			        && ((requiredResult == null && (ResultUtil.isTrue(result) || result.getValue() == null)) || (result != null && result
			                .getValue().equals(requiredResult)));
			boolean dateOutsideResultFinder = context != null
			        ...
			        && !dateBetween((Date) context.getParameterValue("startDate"),
			            (Date) context.getParameterValue("endDate"), (Date) result.getValue());
	...
	public static boolean dateBetween(Date startDate, Date endDate, Date dateInQuestion) {
		return startDate == null || endDate == null || dateInQuestion == null ? false : (dateInQuestion.equals(startDate)
		        || dateInQuestion.equals(endDate) || (startDate.compareTo(dateInQuestion)
		        * dateInQuestion.compareTo(endDate) >= 0));
	}
	...
	public static boolean anyObsBetween(List<Obs> vlObsList, Date startDate, Date endDate) {
		for (Obs obs : vlObsList) {
			if (obs.getObsDatetime() != null && obs.getObsDatetime().compareTo(startDate) >= 0
