package org.openmrs.module.ehraddons.reporting.library.queries.special_clinic;

public class SpecialClinicCohorts {
	
	public static String getSpecialClinic(int conceptId, int ans) {
		String sql = "SELECT p.patient_id FROM patient p INNER JOIN encounter e ON p.patient_id=e.patient_id INNER JOIN "
	...
	public static String getAllVisits(int patientState) {
		String sql = "SELECT p.patient_id FROM patient p INNER JOIN encounter e ON p.patient_id=e.patient_id INNER JOIN "
		...
		return String.format(sql, patientState);
	}
}
